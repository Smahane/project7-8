...
public class Wave extends Condition {
	...
	private void advance() {
		distanceTraveled += schiet.Utils.bulletVelocity(bulletPower);
	}
	...
	private int currentBin() {
		int bin = (int) Math
				.round(((Utils.normalRelativeAngle(schiet.Utils.absoluteBearing(
						gunLocation, targetLocation) - bearing)) / (lateralDirection * BIN_WIDTH))
						+ MIDDLE_BIN);
		
		return schiet.Utils.minMax(bin, 0, BINS - 1);
	}
